...
namespace AISTN.Common.Services
{
    ...
    public class AccountService : ServiceBase
    {
        ...
        /// <summary>
        /// This method is only used when we have already been authenticated with eAuthentication
        /// </summary>
        /// <param name="citizenshipNumber"></param>
        /// <param name="password"></param>
        /// <returns></returns>
        public OperationResult<string> LoginWithCitizenShipNumber(string citizenshipNumber)
        {
            try
            {
                var user = _userRepository.Get(x => x.Egn == citizenshipNumber, src => src.Include(x => x.Roles)).FirstOrDefault();

                if (user == null)
                {
                    return Exception<string>(new Exception("Няма намерен потребител."));
                }
