<template> 
  <div>
    ...
    <entity-modal ref="entityModal" @entitySaved="setEntity" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addNewEntity(){
      this.$refs.entityModal.openModal();
    },
    setEntity(entityData){
      let query = {pageSize: 10, pageNumber: 1, searchCriteria: null, entityId: entityData.id}
      apiMetaDataGetEntities(query).then((result) => {
        this.$set(this, "filteredResults", [...this.filteredResults, ...result.items]);
        this.internalValue = this.selected = entityData.id;
        this.$refs.entityModal.closeModal();
      });
    }
