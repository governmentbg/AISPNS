    ...
    >
      ...
      <template v-slot:item="{ item }">
        <v-list-item-content>
          ...
          <v-list-item-subtitle v-html="'ЕГН:' + item.egn" v-if="item.egn">
          </v-list-item-subtitle>
        </v-list-item-content>
      </template>
    </v-autocomplete>
    <person-modal ref="personModal" @personSaved="setPerson" />
  </div>
</template>
...
import { apiMetaDataGetPersons } from '@/api/metaData';
import { PersonModal } from '@/components';
export default {
  components: { 
    PersonModal 
  },
  name: 'base-person-autocomplete',
  mixins: [Proxyable],
  props: {
    label: {
      type: String,
  ...
  methods: {
    searchSyndics(search = ""){
      ...
      apiMetaDataGetPersons(query).then(result => {
    ...
    setPerson(personData){
      let query = {pageSize: 10, pageNumber: 1, searchCriteria: null, personId: personData.id}
      apiMetaDataGetPersons(query).then((result) => {
        this.$set(this, "filteredResults", [...this.filteredResults, ...result.items]);
        this.internalValue = this.selected = personData.id;
