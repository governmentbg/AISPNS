<template>
  <v-dialog v-model="open" width="70%" scrollable>
    ...
    <property-file-upload-modal ref="propertyUploadFileModal" :parent-data="data" @reloadData="getDocumentsListData" @setDocumentCollectionId="setDocumentCollectionId" />
  </v-dialog>
</template>
...
import { apiGetPropertyById, apiCreateProperty, apiUpdateProperty } from "@/api/properties";
...
import { PropertyFileUploadModal } from "@/components";
...
export default {
  ...
  methods: {
    getData() {
      apiGetPropertyById(this.data.id).then((result) => {
        if(result){
    ...
    save(closeModal = false) {
      if(this.$refs.form.validate())
        if (this.isNewDoc) {
          apiCreateProperty(this.data).then((result) => {
          ...
          apiUpdateProperty(this.data).then((result) => {
            if (result && result.id) {
              this.$emit("update");
              if(closeModal)
                this.closeModal();
    ...
    addNewFile(){
      this.$refs.propertyUploadFileModal.openModal();
    },
