...
import { apiCreateCourseApplication, apiGetCourseApplicationById } from "@/api/courses";
import { apiGetProgramCourses } from "@/api/programs";
import { apiGetCurrentPrograms } from "@/api/metaData";
import { ISODateString } from "@/utils";
export default {
  name: "courseApplicationModal",
  props: {
    syndicData: {
      type: Object,
      default: () => null,
  ...
  methods: {
    getData() {
      apiGetCourseApplicationById(this.data.id).then((result) => {
    ...
    save(closeModal = false) {
      if(this.$refs.form.validate() && this.isNewDoc)
        apiCreateCourseApplication(this.data).then((result) => {
    ...
    getPrograms(){
      apiGetCurrentPrograms().then(result => {
        this.$set(this.nomenclatures, "programs", result);
      });
    },
