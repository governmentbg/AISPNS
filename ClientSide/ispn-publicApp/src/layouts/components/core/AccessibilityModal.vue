  ...
  >
    <v-card id="accessibilityModal" ref="accessibilityModal">
      ...
      <v-card-text class="pt-10">
        ...
        <v-row>
          <v-col cols="12">
            Цветова гама на сайта:
            <v-btn
              outlined
              :color="$vuetify.theme.isDark ? 'black' : 'primary'"
              small
              class="mx-3"
              @click="setLightTheme"
              :style="`background-color: white;border: 1px solid black; ${$vuetify.theme.isDark ? 'black--text' : ''}`"
            >
              Светла
            </v-btn>
            <v-btn
              ...
              small
              @click="setDarkTheme"
              :style="`background-color: black;border: 1px solid ${$vuetify.theme.isDark ? 'white' : 'black'}`"
            >
              Тъмна
            </v-btn>
          </v-col>
