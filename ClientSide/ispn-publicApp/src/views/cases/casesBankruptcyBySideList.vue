<template>
  <v-container fluid tag="section" class="full-height">
    ...
    <cases-list-filter ref="casesBankruptcyListFilter" :search-by-case="false" @doFilter="getData" class="mt-5" />
...
import { apiGetCasesByEntity, apiGetCasesByPerson } from "@/api/cases";

export default {
  name: "casesBankruptcyBySideList",
  components: {
    CasesListFilter,
  },
  data: () => ({
  ...
  methods: {
    getData(clear = false) {
      ...
      if(!clear){
        ...
        if(this.$refs.casesBankruptcyListFilter.personType === 0){
