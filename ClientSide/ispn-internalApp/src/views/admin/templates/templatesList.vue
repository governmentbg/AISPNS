<template>
  <v-container fluid tag="section" class="full-height">
    ...
    <template-modal ref="templateModal" @reloadData="getData" @confirm="confirm" @closeConfirm="$refs.confirmModal.closeModal()" />
    <base-confirm-modal ref="confirmModal" />
  </v-container>
</template>

<script>
import { apiDownloadAttachedTemplateDocument, apiGetTemplateDocuments, apiDeleteTemplateDocument } from "@/api/administration";
...
export default {
  ...
  data: () => ({
    table: {
      headers: [
        { title: "Тип шаблон", field: "description", sortable: true },
        { title: "Качен на", field: "dateCreated", type: 'date', format: '{0:dd.MM.yyyy}', sortable: false },
        {
          title: "",
          cell: "actions",
          filterable: false,
          ...
          buttons: [
            ...
            { title: 'Свали шаблона', icon: 'mdi-download', color: 'white', class: 'secondary mr-1', click: 'download' },
            { title: 'Изтрий шаблона', icon: 'mdi-trash-can-outline', color: 'white', class: 'error', click: 'delete' },
          ],
        },
      ],
      data: [],
  ...
  methods: {
    getData() {
      ...
      apiGetTemplateDocuments(query).then((result) => {
    ...
    tableActions({ action, rowData }) {
      switch (action) {
        ...
        break;
        case "delete":
          this.confirmDeleteTemplate(rowData.id)
        break;
      }
    },
    addNew(){
    ...
    downloadFile(rowData) {
      apiDownloadAttachedTemplateDocument(rowData.id).then((response) => {
    ...
    deleteTemplateFile(fileId) {
      apiDeleteTemplateDocument(fileId).then(() => {
