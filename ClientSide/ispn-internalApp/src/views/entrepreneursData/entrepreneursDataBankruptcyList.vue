        ...
        <v-tabs-items v-model="currentTab">
          <template v-for="tab in tabs">
            <v-tab-item eager :value="tab.key" :key="tab.key" class="mt-5">
              ...
              <entrepreneurs-bankruptcy-filter :ref="tab.key + 'entrepreneursDataListFilter'" :courts="nomenclatures.courts" @doFilter="getTablesData" />
              <base-kendo-grid
...
<script>
import { EntrepreneursBankruptcyFilter } from "@/components";
...
export default {
  ...
  data: () => ({
    ...
    tabs: [
      ...
      { name: "Неподлежащи на вписване", key: "notSubjectToAnnouncement" }
    ],
    nomenclatures: {
      courts: []
    },
  ...
  methods: {
    getTablesData() {
      this.$nextTick(() => {
        ...
        query.filter = Object.assign({isStabilization: false}, this.$refs[this.currentTab+'entrepreneursDataListFilter'].filters);
    ...
    preview(item) {
      this.$router.push({ path: `/entrepreneurs-data/bankruptcy/${item.id}` });
    },
