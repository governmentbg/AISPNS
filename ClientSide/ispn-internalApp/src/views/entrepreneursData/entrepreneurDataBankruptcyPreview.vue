<template>
  ...
  >
    <base-v-component
      heading="Вписване на данни по несъстоятелност на предприемач"
      goBackText="Обратно към вписване на данни предприемачи несъстоятелност"
      goBackTo="/entrepreneurs-data/bankruptcy"
    />
    ...
    <v-row class="mt-5 d-print-none">
      ...
      <v-col cols="12" lg="7" xl="8" class="text-right">
        ...
        <v-btn
          color="secondary"
          @click="$router.push(`/entrepreneurs-acts/bankruptcy/${actData.id}`)"
          v-if="!isReadonly"
        >
      ...
      <v-col cols="12">
        ...
        >
          <template v-slot:after-heading>
            Вписване на данни по несъстоятелност предприемач
          </template>
    ...
    <entrepreneur-bankruptcy-register-modal ref="entrepreneurBankruptcyRegisterModal" @update="getEntrepreneurRegistersData" :act-data="actData" @confirm="getConfirm" @closeConfirm="$refs.confirmModal.closeModal()" />
...
import { ISODateString, deepClone, downloadFileFromResponse, getFullName } from '@/utils';
...
import { EntrepreneurBankruptcyRegisterModal } from "@/components";
import { eActStatuses } from "@/utils/enums/enumerators";
import { isReadonly } from 'vue';
export default {
  name: "entrepreneurDataBankruptcyPreview",
  components: {
  ...
  mounted(){
    ...
    if(this.$route.params.id != 'create'){
      ...
      this.$router.push('/entrepreneurs-data/bankruptcy');
    }
  },
  computed: {
    ...mapGetters([
    ...
    isReadonly(){
      return this.data.registrationStatusId?.toLowerCase() !== eActStatuses.NOT_PROCESSED.toLowerCase();
    },
  },
  watch: {
    isReadonly: {
  ...
  methods: {
    ...
    previewEntryData(rowData){
      this.$refs.entrepreneurBankruptcyRegisterModal.openModal(rowData.id);
    },
    ...
    getEntrepreneurRegistersData(){
      ...
      apiGetAllRegisterEntries(query).then(result => {
        ...
        this.$set(this.table, 'data', result.items.sort((a, b) => {
    ...
    addNew(){
      this.$refs.entrepreneurBankruptcyRegisterModal.openModal()
    },
