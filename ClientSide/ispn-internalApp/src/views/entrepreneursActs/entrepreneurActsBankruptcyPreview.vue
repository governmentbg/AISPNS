<template>
  ...
  >
    <base-v-component
      heading="Обявяване на актове по несъстоятелност на предприемач"
      goBackText="Обратно към обявяване на актове предприемачи несъстоятелност"
      goBackTo="/entrepreneurs-acts/bankruptcy"
    />
      ...
      <v-col cols="12">
        ...
        >
          <template v-slot:after-heading>
            Обявяване на актове по несъстоятелност предприемач
          </template>
...
export default {
  name: "entrepreneurActsBankruptcyPreview",
  components: {
  },
  data(){
    return {
  ...
  mounted(){
    ...
    if(this.$route.params.id != 'create'){
      ...
      this.$router.push('/entrepreneurs-acts/bankruptcy');
    }
  ...
  methods: {
    ...
    getActCategories(){
      apiMetaDataGetActAnnouncementCategories(false).then(result => {
        this.$set(this.nomenclatures, "actCategories", result);
      })
    },
    announceActAsk(){
