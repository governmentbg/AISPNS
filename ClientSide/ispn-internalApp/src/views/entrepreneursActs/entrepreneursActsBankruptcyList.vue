        ...
        <v-tabs-items v-model="currentTab">
          <template v-for="tab in tabs">
            <v-tab-item eager :value="tab.key" :key="tab.key" class="mt-5">
              ...
              <entrepreneurs-bankruptcy-acts-filter :ref="tab.key + 'entrepreneursActsListFilter'" :courts="nomenclatures.courts" @doFilter="getTablesData" />
              <base-kendo-grid
...
<script>
import { EntrepreneursBankruptcyActsFilter } from "@/components";
  ...
  methods: {
    getTablesData() {
      this.$nextTick(() => {
        ...
        query.filter = Object.assign({isStabilization: false}, this.$refs[this.currentTab+'entrepreneursActsListFilter'].filters);
    ...
    preview(item) {
      this.$router.push({ path: `/entrepreneurs-acts/bankruptcy/${item.id}` });
    },
    getCourts(){
    ...
    addNew(){
      this.$router.push({path: `/entrepreneurs-acts/bankruptcy/create`})
    },
  },
};
</script>
