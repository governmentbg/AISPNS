<template>
  <v-container fluid tag="section" class="full-height">
    ...
    <v-row class="my-5 d-print-none">
      <v-col cols="12" class="text-right">
        <v-btn
          ...
          @click="generateSyndicsList"
        >
          <v-icon left dark>mdi-refresh</v-icon>
          Генериране списък на синдици
        </v-btn>
        <v-btn
          class="grey darken-3 white--text"
          @click="generateSyndicsListWhoNotPaid"
        >
          <v-icon left dark>mdi-currency-usd-off</v-icon>
          Генериране списък с лицата, незаплатили вноска
        </v-btn>
        <v-btn
          class="grey darken-1 white--text"
          @click="generateSyndicsAppeal"
        >
    ...
    <syndics-list-filter ref="syndicsListFilter" @doFilter="getData" />
...
<script>
import { apiExportActiveSyndicsToExcel, apiExportActiveSyndicsWithoutPaidInstallmentToExcel, apiSearchSyndics } from "@/api/syndics";
...
export default {
  ...
  methods: {
    ...
    addNew(){
      this.$router.push({path: `/syndics/create`})
    },
    generateSyndicsList(){
      apiExportActiveSyndicsToExcel().then(result => {
        result && downloadFileFromResponse(result)
      })
    },
    generateSyndicsListWhoNotPaid(){
      apiExportActiveSyndicsWithoutPaidInstallmentToExcel().then(result => {
        result && downloadFileFromResponse(result)
      })
    },
    generateSyndicsAppeal(){
