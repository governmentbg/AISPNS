  ...
  <v-dialog v-model="open" width="80%" scrollable>
    <v-card>
      <v-card-title class="headline">
        <template v-if="isNewDoc">Ново вписване на данни на предприемач по стабилизация</template>
        <template v-else>Преглед на вписани данни на предприемач по стабилизация</template>
      </v-card-title>
...
export default {
  name: "entrepreneurStabilizationRegistration",
  components: {
  },
  props: {
    actData: {
  ...
  methods: {
    ...
    getActFields(){
      apiMetaDataGetActFields(true).then(result => {
