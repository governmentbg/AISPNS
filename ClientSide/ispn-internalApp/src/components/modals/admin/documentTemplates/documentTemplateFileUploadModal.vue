...
export default {
  name: "sellAnnouncementFileUploadModal",
  props: {
    type: {
      type: String,
      default: null
  ...
  methods: {
    ...
    save(closeModal = false) {
      if(this.$refs.form.validate()){
        ...
        let documentTypeGuid = eDocumentContentTypes.TEMPLATE_DIRECTIVE28;

        if(this.type === 'syndicTemplates'){
          documentTypeGuid = eDocumentContentTypes.TEMPLATE_SYNDIC;
        } else if(this.type === 'syndicReports'){
          documentTypeGuid = eDocumentContentTypes.TEMPLATE_REPORT_SYNDIC;
        }

        formData.append("documentContentType", documentTypeGuid);
