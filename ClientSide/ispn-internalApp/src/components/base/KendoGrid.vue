    ...
    <template v-slot:actions="{props}">
      <td :class="props.className ? props.className + ' text-center' : 'text-center 123'">
        <template v-for="(button, bIdx) in buttons">
          ...
          <template v-if="!Object.prototype.hasOwnProperty.call(button, 'if') ? !Object.prototype.hasOwnProperty.call(button, 'if_eq') ? true : checkButtonVisibilityEq(button, props.dataItem) : checkButtonVisibility(button, props.dataItem)">
...
export default {
  ...
  methods: {
    ...
    checkButtonVisibilityEq(button, rowData){
      return rowData[button.if_eq.field] === button.if_eq.value;
    },
